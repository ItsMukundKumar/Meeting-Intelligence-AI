...
def _transcribe_chunk_local(chunk_path: str, translate: bool = False) -> str:
    model = load_model()
    task = "translate" if translate else "transcribe"
    segments, _ = model.transcribe(chunk_path, task=task, beam_size=5, vad_filter=True)
...
def transcribe_chunk(chunk_path: str, translate: bool = False) -> str:
    if USE_GROQ:
        return _transcribe_chunk_groq(chunk_path)
    return _transcribe_chunk_local(chunk_path, translate)
...
def transcribe_all_chunks(chunks: list, translate: bool = False) -> str:
    ...
    for i, chunk in enumerate(chunks):
        ...
        full_transcript += transcribe_chunk(chunk, translate) + " "
...
def transcribe(chunks: list, language: str = "en", translate: bool = False) -> str:
    ...
    return transcribe_all_chunks(chunks, translate=translate)
